...
add('RI-02', 'Canonical release name', identity.release_name === 'VNFINANCE-FPA-v1.1.0', identity.release_name);
add(
  'RI-03',
...
add(
  ...
  identity.release_status === 'RECRUITER_READY_WITH_OPEN_EXTERNAL_CLAIM_GATES'
    && identity.release_tag === 'fpa-portfolio-v1.1.0'
...
add('RI-08', 'Manifest carries release identity family', manifest.includes('VNFINANCE-FPA') && manifest.includes('Power BI'), 'release family and scope present');
add('RI-09', 'Handoff carries release identity family', handoff.includes('VNFINANCE') && handoff.includes('Power BI'), 'release family and scope present');
...
const outputPath = path.join(ROOT, 'reports', 'RELEASE_IDENTITY_NONBI_QA_V1.1.0_2026-09-08.json');
fs.writeFileSync(outputPath, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(JSON.stringify({ status: report.status, checks: checks.length, passed, report: 'reports/RELEASE_IDENTITY_NONBI_QA_V1.1.0_2026-09-08.json' }, null, 2));
