...
def main() -> int:
    ...
    add("health query has freshness status", "control_status" in health_query and "control_reason" in health_query and "latency_minutes" in health_query)
    add("health query distinguishes freshness reasons", all(token in health_query for token in ["N'CURRENT'", "N'STALE_WATERMARK'", "N'REJECTED_ROWS'", "N'LOAD_FAILED'", "N'NO_LOAD'"]))
