...
const N=5000, rows=[]; for(let i=0;i<N;i++) rows.push({revenue:80.1+3*norm(),ebitda_proxy:58.5+3.8*norm(),ebitda_margin:29.7+1.8*norm(),ccc:Math.max(20,54.8+7*norm())});
const pct=(key,q)=>{const a=rows.map(r=>r[key]).sort((x,y)=>x-y);return a[Math.floor(q*(a.length-1))];};
const defs=[["revenue","VND_bn",76.9,"below"],["ebitda_proxy","VND_bn",53.1,"below"],["ebitda_margin","percent",25,"below"],["ccc","days",68.8,"above"]];
