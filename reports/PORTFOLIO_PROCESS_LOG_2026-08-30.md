...
The package source used for the latest Drive sync is GitHub commit `5073ef5` (fixture parity CI and preflight). CI run `33317607840` passed. The Drive bundle was updated in place at file ID `1PAOAS0D60Ueh20b26i9MqBaZB9st3tiX` and includes the PBIP/PBIT source, the same committed current synthetic CSVs, DirectQuery schema, validators, CI workflow and runbook/checklist. A follow-up preflight enhancement allowing `-DesktopPath` was committed at GitHub commit `457520e`; the bundle should be re-uploaded after this documentation update. The bundle is remote-authoritative; the temporary clone used for packaging is deleted after verification.
