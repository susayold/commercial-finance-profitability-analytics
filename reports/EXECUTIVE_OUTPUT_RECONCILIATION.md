...
| Working-capital priority | SIMULATED + DERIVED | `Working_Capital`, `AR`, `Inventory`, `AP` | DSO + DIO − DPO = CCC using the monthly denominator convention; QA-11 | Illustrative FY2025 case is 54.8 days average CCC; June escalation is a model flag, not observed cash performance |
...
| Forecast Bias / WAPE | DERIVED specification | `docs/FORECAST_ACCURACY_BACKTEST.md`; `scripts/compute_forecast_accuracy.mjs`; native Google Sheet | Only `ELIGIBLE` + `FROZEN` snapshots enter the calculation; deterministic unit fixture matches expected output | No live VietNova Bias/WAPE is claimed yet because no pre-close frozen snapshot history exists |
