# Reconciled `final_v1` finance-model contract
...
The v2 workbook's invoice ledger stores gross revenue and unit price at a scaled invoice level while Units and COGS remain at the pre-scale decimal. For example, `VND 186m / VND 31k = 6,000 units`, not `0.06`. `build_powerbi_final_v1.py` corrects the physical units and allocates preliminary SKU COGS to the authoritative month × channel COGS total before any downstream finance artifact is built.
